...
export default function MerryChristmas2() {
    ...
    // ==========================================
    // 3. window.THREE.JS SYSTEM Vandiep
    // ==========================================
    function init3D() {
        const container = document.getElementById('canvas-container');
        scene = new window.THREE.Scene();
        scene.fog = new window.THREE.FogExp2(0x000000, 0.002);
    ...
    function createParticleSystem(type, count, size) {
        ...
        geo.setAttribute('position', new window.THREE.Float32BufferAttribute(pPositions, 3));
        geo.setAttribute('size', new window.THREE.Float32BufferAttribute(sizes, 1));

        // Init colors Vandiep
        const colors = new Float32Array(count * 3);
        const baseColor = new window.THREE.Color();
    ...
    function createPhotos() {
        const geo = new window.THREE.PlaneGeometry(8, 8);
        const borderGeo = new window.THREE.PlaneGeometry(9, 9);
        const borderMat = new window.THREE.MeshBasicMaterial({ color: 0xFFD700 });

        for (let i = 0; i < 5; i++) {
            const mat = new window.THREE.MeshBasicMaterial({ map: photoTextures[i], side: window.THREE.DoubleSide });
            const mesh = new window.THREE.Mesh(geo, mat);
            const border = new window.THREE.Mesh(borderGeo, borderMat);
    ...
    function createDecorations() {
        ...
        const mat = new window.THREE.MeshBasicMaterial({ map: tex, transparent: true, blending: window.THREE.AdditiveBlending });
        titleMesh = new window.THREE.Mesh(new window.THREE.PlaneGeometry(60, 15), mat);
        ...
        const starTex = new window.THREE.CanvasTexture(starCanvas);
        const starMat = new window.THREE.MeshBasicMaterial({ map: starTex, transparent: true, blending: window.THREE.AdditiveBlending });
        starMesh = new window.THREE.Mesh(new window.THREE.PlaneGeometry(12, 12), starMat);
        ...
        const loveTex = new window.THREE.CanvasTexture(loveCanvas);
        const loveMat = new window.THREE.MeshBasicMaterial({ map: loveTex, transparent: true, blending: window.THREE.AdditiveBlending });
        loveMesh = new window.THREE.Mesh(new window.THREE.PlaneGeometry(70, 18), loveMat);
    ...
    function animate() {
        ...
        if (state === 'TREE') {
            ...
            titleMesh.scale.lerp(new window.THREE.Vector3(1, 1, 1), 0.1);
            starMesh.rotation.z -= 0.02;
            starMesh.material.opacity = 0.7 + 0.3 * Math.sin(time * 5);
            photoMeshes.forEach(m => { m.scale.lerp(new window.THREE.Vector3(0, 0, 0), 0.1); m.visible = false; });
            ...
            photoMeshes.forEach((mesh, i) => {
                ...
                mesh.position.lerp(new window.THREE.Vector3(x, y, z), 0.1);
                mesh.lookAt(camera.position);
                if (z > maxZ) { maxZ = z; bestIdx = i; }
                if (z > 5) {
                    const ds = 1.0 + (z / CONFIG.photoOrbitRadius) * 0.8;
                    mesh.scale.lerp(new window.THREE.Vector3(ds, ds, ds), 0.1);
                } else {
                    mesh.scale.lerp(new window.THREE.Vector3(0.6, 0.6, 0.6), 0.1);
                }
            });
            ...
            photoMeshes.forEach((mesh, i) => {
                if (i === selectedIndex) {
                    mesh.position.lerp(new window.THREE.Vector3(0, 0, 60), 0.1);
                    mesh.scale.lerp(new window.THREE.Vector3(5, 5, 5), 0.1);
                    mesh.lookAt(camera.position); mesh.rotation.z = 0;
                } else {
                    mesh.scale.lerp(new window.THREE.Vector3(0, 0, 0), 0.1);
                }
            });
        }
